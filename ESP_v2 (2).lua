...
local SIZES={S=520,M=700,L=860,XL=1020}
local currentSize="M"
local WIN_W,WIN_H=700,390
local win=Instance.new("Frame",ScreenGui); win.Name="Window"
win.Size=UDim2.new(0,WIN_W,0,WIN_H); win.Position=UDim2.new(0.5,-WIN_W/2,0.3,0)
...
sidebar.BackgroundColor3=Color3.fromRGB(24,24,24); sidebar.BorderSizePixel=0
...
local function addTopbar(tab,subs)
    ...
    bar.BackgroundColor3=Color3.fromRGB(30,30,30); bar.BorderSizePixel=0; bar.Visible=(tab==currentTab)
    ...
    for _,sub in ipairs(subs) do
        ...
        btn.MouseButton1Click:Connect(function() switchSub(tab,sub[2]) end)
        x+=90
    end
end
...
local function makeCols(parent)
    ...
    frame.Size=UDim2.new(1,0,1,-36); frame.BackgroundTransparency=1; frame.BorderSizePixel=0
...
local function makeCheckbox(parent,initVal,onChange)
    ...
    apply(initVal)
    box.MouseButton1Click:Connect(function() apply(not val) end)
    return box, apply
end
...
do
    ...
    rb.MouseButton1Click:Connect(function() SA.FOV=30; setSliderVal("sa_fov",30); setSliderVal("sa_fov2",30) end)
...
aimIcon.MouseButton1Click:Connect(function()  switchTab("aim")  end)
espIcon.MouseButton1Click:Connect(function()  switchTab("esp")  end)
miscIcon.MouseButton1Click:Connect(function() switchTab("misc") end)
cfgIcon.MouseButton1Click:Connect(function()  switchTab("cfg")  end)
switchTab("aim")

local closeBtn=Instance.new("TextButton",win); closeBtn.Size=UDim2.new(0,20,0,20)
closeBtn.Position=UDim2.new(1,-26,0,8); closeBtn.BackgroundColor3=Color3.fromRGB(58,16,16)
closeBtn.BorderSizePixel=0; closeBtn.Text="✕"; closeBtn.TextSize=10; closeBtn.Font=Enum.Font.GothamBold
closeBtn.TextColor3=Color3.fromRGB(255,85,85); closeBtn.ZIndex=100
Instance.new("UICorner",closeBtn).CornerRadius=UDim.new(0,4)
Instance.new("UIStroke",closeBtn).Color=Color3.fromRGB(102,34,34)
...
do
    ...
    win.InputBegan:Connect(function(inp)
        if inp.UserInputType==Enum.UserInputType.MouseButton1 or inp.UserInputType==Enum.UserInputType.Touch then
            if (inp.Position.Y-win.AbsolutePosition.Y)<=44 then
...
local mob=Instance.new("TextButton",ScreenGui); mob.Size=UDim2.new(0,48,0,48); mob.Position=UDim2.new(1,-60,1,-70)
mob.BackgroundColor3=Color3.fromRGB(20,20,35); mob.BorderSizePixel=0; mob.TextColor3=Color3.fromRGB(200,200,255)
mob.Text="☰"; mob.TextSize=22; mob.Font=Enum.Font.GothamBold; mob.ZIndex=1000
...
mob.InputBegan:Connect(function(inp)
    if inp.UserInputType==Enum.UserInputType.MouseButton1 or inp.UserInputType==Enum.UserInputType.Touch then
        mobDrag=true; mobDragStart=inp.Position; mobStartPos=mob.Position
    end
end)
UserInputService.InputChanged:Connect(function(inp)
    ...
    if inp.UserInputType==Enum.UserInputType.MouseMovement or inp.UserInputType==Enum.UserInputType.Touch then
        ...
        mob.Position=UDim2.new(mobStartPos.X.Scale,mobStartPos.X.Offset+d.X,mobStartPos.Y.Scale,mobStartPos.Y.Offset+d.Y)
    end
end)
UserInputService.InputEnded:Connect(function(inp)
    if inp.UserInputType==Enum.UserInputType.MouseButton1 or inp.UserInputType==Enum.UserInputType.Touch then
        if mobDragStart and (inp.Position-mobDragStart).Magnitude<5 then setMenu(not menuVisible) end
        mobDrag=false; mobDragStart=nil
    end
end)
