...
local function addTopbar(tab,subs)
    ...
    for _,sub in ipairs(subs) do
        ...
        btn.BackgroundTransparency=1; btn.BorderSizePixel=0; btn.Text=sub[1]; btn.TextSize=12; btn.Font=Enum.Font.GothamBold; btn.ZIndex=30
...
-- ── Крестик — внутри sidebar, поверх всего ──────────────
...
closeBtn.TextColor3=Color3.fromRGB(255,85,85); closeBtn.ZIndex=9999
closeBtn.Visible=false
Instance.new("UICorner",closeBtn).CornerRadius=UDim.new(0,6)
Instance.new("UIStroke",closeBtn).Color=Color3.fromRGB(140,34,34)
closeBtn.Activated:Connect(function() setMenu(false) end)

-- Позиция крестика — правый верхний угол окна
RunService.Heartbeat:Connect(function()
    if not menuVisible then closeBtn.Visible=false; return end
    closeBtn.Visible=true
    local ap=win.AbsolutePosition; local as=win.AbsoluteSize
    closeBtn.Position=UDim2.new(0,ap.X+as.X-32,0,ap.Y+4)
end)

-- ── Drag — только через sidebar (не мешает кнопкам внутри страниц) ──
do
    local dragging=false
    local dragStartPos=Vector2.new(0,0)
    local winStartPos=UDim2.new(0,0,0,0)

    -- Отслеживаем тач/мышь глобально
    -- Начало drag только если тап попал в sidebar
    UserInputService.InputBegan:Connect(function(inp,gp)
        if gp then return end
        if not menuVisible then return end
        local utype=inp.UserInputType
        if utype~=Enum.UserInputType.MouseButton1 and utype~=Enum.UserInputType.Touch then return end
        -- Проверяем что тап попал именно в сайдбар
        local pos=inp.Position
        local sbPos=sidebar.AbsolutePosition
        local sbSize=sidebar.AbsoluteSize
        local inSidebar = pos.X>=sbPos.X and pos.X<=sbPos.X+sbSize.X
                       and pos.Y>=sbPos.Y and pos.Y<=sbPos.Y+sbSize.Y
        if not inSidebar then return end
        -- Проверяем что не попали в иконку
        local hitIcon=false
        for _,icon in ipairs({aimIcon,espIcon,miscIcon,cfgIcon}) do
            local ip=icon.AbsolutePosition; local is=icon.AbsoluteSize
            if pos.X>=ip.X and pos.X<=ip.X+is.X and pos.Y>=ip.Y and pos.Y<=ip.Y+is.Y then
                hitIcon=true; break
            end
        end
        if hitIcon then return end
        dragging=true
    ...
    UserInputService.InputChanged:Connect(function(inp)
        if not dragging then return end
        local utype=inp.UserInputType
        if utype~=Enum.UserInputType.MouseMovement and utype~=Enum.UserInputType.Touch then return end
    ...
    UserInputService.InputEnded:Connect(function(inp)
        local utype=inp.UserInputType
        if utype==Enum.UserInputType.MouseButton1 or utype==Enum.UserInputType.Touch then
            dragging=false
        end
    end)
end
...
mob.Size=UDim2.new(0,52,0,52)
mob.Position=UDim2.new(1,-66,1,-80)
...
local mobDragStart=Vector2.new(0,0)
local mobStartAbs=Vector2.new(0,0)
local mobMoved=false

UserInputService.InputBegan:Connect(function(inp,gp)
    if gp or not mob.Visible then return end
    local utype=inp.UserInputType
    if utype~=Enum.UserInputType.MouseButton1 and utype~=Enum.UserInputType.Touch then return end
    local pos=inp.Position
    local mp=mob.AbsolutePosition; local ms=mob.AbsoluteSize
    if pos.X>=mp.X and pos.X<=mp.X+ms.X and pos.Y>=mp.Y and pos.Y<=mp.Y+ms.Y then
        mobDragging=true; mobMoved=false
        mobDragStart=Vector2.new(pos.X,pos.Y)
        mobStartAbs=Vector2.new(mp.X,mp.Y)
    end
end)

UserInputService.InputChanged:Connect(function(inp)
    if not mobDragging then return end
    local utype=inp.UserInputType
    if utype~=Enum.UserInputType.MouseMovement and utype~=Enum.UserInputType.Touch then return end
    local d=inp.Position-mobDragStart
    if d.Magnitude>8 then
        mobMoved=true
        mob.Position=UDim2.new(0,mobStartAbs.X+d.X,0,mobStartAbs.Y+d.Y)
    end
end)
...
UserInputService.InputEnded:Connect(function(inp)
    ...
    if mobDragging then
        ...
        mobDragging=false; mobMoved=false
    end
end)
